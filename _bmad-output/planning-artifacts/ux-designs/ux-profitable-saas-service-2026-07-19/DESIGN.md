...
- **subscription-chip** — sits beside the credits pill. Five states driven by the subscription status, never user.tier (5.5 D8). All: {rounded.full}, 28px, `px-3`, label {typography.small} 500, `{date}` in `<bdi>` + `tabular-nums` (localized month, Western numerals); mobile `min-h-11`. **Free** (no row) and **expired**: transparent fill, {colors.primary} text, 1px {colors.primary} border, label "Free — Upgrade"; the whole chip is a button opening the upgrade dialog; hover `bg-primary/10`. **Starter (active):** solid {colors.primary} / {colors.primary-foreground}, label "Starter — renews on {date}", NON-interactive span (pill geometry signals meter or status — never a clickable primary action; the header carries the Billing link). **Cancelled:** {colors.warning-container} / {colors.warning-on-container}, label "Cancelled — access until {date}", whole-chip button → the upgrade dialog (its Subscribe CTA is the reactivation path). **Failed renewal:** warning pair, label "Payment failed", whole-chip button → the retry-payment redirect (the persistent banner owns the danger-toned alarm). Warning chips: no fill hover (the ring is the interaction cue); universal 2px ring on interactive states.
- **upgrade-dialog** — the single subscription conversion surface (FR-28; EXPERIENCE.md L127). Base UI Dialog, {rounded.xl}, max-w-md, bg-card; backdrop `bg-black/40`; title in {typography.headline} 24/600; plan block mirrors the pack card — credits readout ("200 credits/mo") in {typography.headline} with the price ("1,500 DZD/mo") beneath in {typography.title}, both `tabular-nums`; 3 inclusions with lucide Check size-4 {colors.success} in {typography.small}; full-width {colors.primary} CTA ("Subscribe via Chargily") → Chargily checkout; busy state swaps to the processing label + `aria-busy`; checkout failure = inline `role="alert"` (dialog stays open); X top-inline-end (`common.actions.close`); initial focus on the CTA; close never redirects; focus returns to the invoking control; host modal closes first (stack depth 1).
...
- **banner** — full-width strip, {rounded.md}, icon at inline-start, message in {typography.small}, optional underlined CTA, dismiss `X` at inline-end **except on the persistent variant**. Variants: **info** ({colors.info-container} / {colors.info-on-container}) — 15-credit welcome, general notices; **warning** ({colors.warning-container} / {colors.warning-on-container}) — reserved for recoverable account friction; **danger** ({colors.danger-container} / {colors.danger-on-container}) — 0-credit state with top-up CTA; **persistent** — the failed-renewal banner (FR-28): {colors.danger-container} / {colors.danger-on-container}, `h-10` 40px single line, `px-4`, lucide `TriangleAlert` size-4 `aria-hidden` at inline-start, message in {typography.small} with the action clause as the underlined on-container link (`font-medium` — never `text-destructive` on the danger fill), `role="alert"`, in-flow below the header on all authenticated surfaces (pushes content; the sticky header stays z-50), non-dismissible (no `X`), no transitions; narrow screens wrap (`min-h-10` growth — never truncate); clears only on resolution (the paid flip or the expiry beat).
