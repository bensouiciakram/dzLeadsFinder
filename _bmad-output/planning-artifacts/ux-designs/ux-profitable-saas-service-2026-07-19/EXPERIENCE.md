...
| Subscription chip | App header (authed) | Five states driven by the SUBSCRIPTION row's status, never user.tier (5.5 D8): **free** (no row) and **expired** — transparent fill, `{colors.primary}` text, 1px `{colors.primary}` border, whole-chip button → the Upgrade dialog; **starter (active)** — solid `{colors.primary}` / `{colors.primary-foreground}` "Starter — renews on {date}" (localized month name + Western numerals in `<bdi>`), non-interactive status readout; **cancelled** — `{colors.warning-container}` / `{colors.warning-on-container}` "Cancelled — access until {date}", whole-chip button → the Upgrade dialog (its Subscribe CTA is the reactivation path); **failed_renewal** — warning pair "Payment failed", whole-chip button → the retry-payment redirect (the persistent banner owns the danger-toned alarm). All `{rounded.full}` 28px, mobile ≥44px (5.7). |
...
| Upgrade dialog | Opened from every upgrade CTA | Single in-app dialog for all entry points (header chip, 0-credit recovery — free users only, watermark modal, xlsx tooltip, search daily-limit state; the export 5,000/24h limit is tier-independent and stays the come-back-tomorrow message). Base UI Dialog `{rounded.xl}` max-w-md; headline title; plan block mirrors the pack card (credits headline + price title, tabular-nums); 3 inclusions with success checks; full-width primary "Subscribe via Chargily" → Chargily checkout (create-checkout); X top-inline-end; initial focus on the CTA; host modal closes first (stack depth 1); close never redirects; focus returns to the invoking control. No public pricing page dependency — the dialog is the sole conversion surface. The Starter 0-credit recovery is the separate Recovery dialog (top-up packs; `{colors.card}` pack cards). |
...
| Pack cards | `/billing` | Two cards side by side: 500 DZD / 75 credits (6.7 DZD/credit) and 1,500 DZD / 250 credits (6.0 DZD/credit); never-expires note on both; Best-value badge in `{colors.primary}` solid / `{colors.primary-foreground}` on the 1,500 DZD card (DESIGN.md pins primary — corrected 2026-08-11, 5.6 doc pass). Free users can buy packs too (FR-25). |
...
| status-card | `/billing` (inline, on return from checkout) | Carries `role="status"` so polling→success/timeout flips are announced even before the toast. Polls up to 60s in "Confirming payment…" state → success flips the card to the credits-posted state (`{colors.success-container}` / `{colors.success-on-container}`) + toast; timeout shows payment-received-credits-post-soon note (info-toned, never an error). A failed settlement flips the card to `{colors.danger-container}` / `{colors.danger-on-container}` with the failure surface (role="alert"); refunded renders the same terminal state. Non-blocking; webhook reconciliation is idempotent (FR-27). |
| banner (persistent variant) | All authenticated surfaces | The failed-renewal banner (FR-28): `{colors.danger-container}` / `{colors.danger-on-container}`, compact 40px, in-flow below the header (pushes content), `role="alert"`, lucide alert icon at inline-start, non-dismissible (no `X` — persists until payment succeeds); the AC sentence renders as rich text with the second clause ("update your payment method to keep Starter") as the underlined on-container link → the retry-payment redirect (create-checkout; Chargily V1 has no customer-portal URL — the settings-pinned constant is a pre-prod gate). Prior-cycle credits remain usable until the next cycle would have begun (enforced by the expiry beat). Clears only on resolution. |
...
| Payment-failed (renewal) | All authenticated surfaces | Declined renewal | Persistent non-dismissible banner (persistent variant); prior-cycle credits usable until next cycle would have begun (FR-28). At `current_period_end` the daily expiry beat lapses the row to **expired** (pool zeroed, `user.tier` → free — the 5.7 cancel sync); the banner clears (no "keep Starter" promise on an ended subscription); the resubscribe path (Upgrade dialog) takes over. |
...
- **Failed renewal lifecycle (FR-28):** decline → persistent non-dismissible banner on every app surface with a Chargily update-payment link (the retry-payment redirect at V1); prior-cycle credits remain usable until the next cycle would have begun; banner clears only on resolution (a paid renewal flips the row active; at period end the daily beat lapses the row to **expired** — pool zeroed + `user.tier` → free — and the banner clears); no new grant until payment succeeds.
- **Cancellation (FR-26):** self-serve from the danger zone with an access-until-{date} confirmation; no refund of the current cycle; `/refund-policy` documents the no-refund-by-default stance (documented payment errors handled by ops manually). A cancelled row is never auto-expired mid-cycle (5.5 D14); after period end the 5.7 beat expires it the same way as failed_renewal; reactivation before period end re-anchors at `max(current_period_end, now)` (5.5 D19); `user.tier` stays 'starter' until the expiry sync (5.7) — every surface reads the subscription status, never the tier (5.5 D8).
