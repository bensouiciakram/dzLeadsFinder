...
- [x] **Task 3: i18n keys ×3 locales** (AC: aria-label never placeholder-only; D5)
  - [x] 3.1 `frontend/messages/en.json` — UPDATE — REUSE existing keys: `search.filters.wilaya` (group label — rendered by the sidebar h3 today), `search.filters.wilaya_placeholder` (combobox input placeholder; value "Select wilayas..." works as-is), `trust.wilayas.no_results` ("No wilayas match your search" — the combobox empty state; the key lives under the `trust` namespace, used by the /wilayas page filter). NEW keys (working drafts — native-speaker review is a pre-launch item, PRD Open Q7):
...
- [x] [Review][Patch] Base UI 1.6.0 DOES handle Backspace chip removal natively (source-verified `ComboboxInput.js:357-360`) — the custom handler double-fired per keypress and, beyond CHIP_LIMIT=3, removed the WRONG chip (native uses `renderedChipsCount - 1` = 3rd visible chip). [WilayaCombobox.tsx] — FIXED: handler now `preventDefault()` + `preventBaseUIHandler()` so exactly one removal with last-selected semantics; 4-chip test asserts one call + correct code. Story D3's "not native" claim corrected (dev record).
- [x] [Review][Patch] Esc with the popup CLOSED silently wipes the ENTIRE selection (native Base UI branch, source-verified `ComboboxInput.js:344-349` — `!mounted && Escape` → `setSelectedValue([])`). [WilayaCombobox.tsx] — FIXED: Esc-on-closed-popup intercepted (`preventDefault` + `preventBaseUIHandler` + query cleared), selection preserved; test.
- [x] [Review][Patch] IME composition: Base UI suppresses `onInputValueChange` during composition, so `query === ''` while Arabic text is composing — Backspace deleted a CHIP instead of composing text (Arabic-market critical). [WilayaCombobox.tsx] — FIXED: composition guard (`nativeEvent.isComposing`/synthetic `isComposing`) suppresses the native removal on Backspace/Delete without preventDefault (text editing unaffected); test.
- [x] [Review][Patch] Wrong i18n key path: `ComboboxEmpty` used `t('search.wilayas.no_results')` — the key is `trust.wilayas.no_results` (the key-returning test mock masked a runtime `[MISSING]`). [WilayaCombobox.tsx] — FIXED + `i18n-shape.test.ts` now asserts EVERY rendered key resolves in all 3 locales (en/fr/ar JSON imports).
- [x] [Review][Patch] Popup missing the spec'd accessible name (story Task 4.2 literal). [WilayaCombobox.tsx] — FIXED: `aria-label={t('search.filters.wilaya')}` on `ComboboxContent`.
- [x] [Review][Patch] Keyboard-active fill: D4 spec'd `data-highlighted:bg-muted` (DESIGN.md) — registry `accent` remained (token-identical today, spec intent unenforced). [WilayaCombobox.tsx] — FIXED: usage-site override appended.
- [x] [Review][Patch] `wilaya_more` count passed as a NUMBER — next-intl v4 formats `{count}` with the locale's numeral system → Arabic-Indic digits in AR (AD-8 violation). [WilayaCombobox.tsx] — FIXED: `String(hiddenCount)` (Western digits guaranteed).
- [x] [Review][Patch] Clear affordance touch target: `min-h-11` with a 16px icon ≈ 16×44px hit area (UX-DR22). [WilayaCombobox.tsx] — FIXED: `size-11 md:size-4`; test.
- [x] [Review][Patch] No focus-visible styling on chip-remove / clear buttons (UA default only). [WilayaCombobox.tsx] — FIXED: `focus-visible:ring-2 focus-visible:ring-ring` on both.
- [x] [Review][Patch] Unknown codes in `value` ([999]): chips/clear/Backspace acted on the raw value (clear button + badge + removal on an invisible chip). [WilayaCombobox.tsx] — FIXED: all derived from the normalized, DEDUPED `selected` (Map-based memo); consistent empty state; duplicate codes no longer collide React keys or remove-all; tests.
- [x] [Review][Patch] Leading-zero code queries (`'01'`, `'031'`) matched nothing — Algerian users write 2-digit codes. [WilayaCombobox.tsx] — FIXED: all-digit queries normalized (`replace(/^0+(?=\d)/, '')`); unit tests.
- [x] [Review][Patch] Parity test was one-way (frontend ⊆ backend): a backend-only 59th wilaya or duplicate backend codes passed; industries compared only `name_en`; parser silently skipped trailing content and could false-match `export const WILAYAS_BY_CODE`. [test_wilaya_parity.py] — FIXED: bidirectional code-set equality + backend duplicate detection + all-3-names equality for industries + trailing-content rejection + anchored probe.
- [x] [Review][Patch] Registry `no-scrollbar` class was a no-op (utility missing from globals.css) → visible scrollbar on the 58-row list. [globals.css] — FIXED: `@utility no-scrollbar` added (registry-intended hidden scrollbar).
- [x] [Review][Defer] Dual combobox instances (aside + drawer copies of the same `wilayaField`) keep independent query state; a drawer-typed query can resurface when the aside popup reopens; two `role="combobox"` inputs exist while the drawer is open (aside is `display:none` so not in the a11y tree). Lift query state or scope by surface in 3.5/3.6. [FilterSidebar.tsx, WilayaCombobox.tsx] — deferred, documented in deferred-work.md.
- [x] [Review][Defer] `elementFromPoint` jsdom polyfill ignores coordinates — with the drawer open AND the combobox popup open, every point reports the combobox popup, so combobox click-outside dismissal is untestable in that combination (latent; no current test opens the combobox inside the drawer). [src/test/mocks.ts] — deferred, documented in deferred-work.md.
- [x] [Review][Defer] Combobox popup can outlive its anchor when the viewport crosses the md breakpoint while open (orphan floating popup; real browser only — the drawer has the matchMedia-close precedent). [WilayaCombobox.tsx] — deferred, documented in deferred-work.md.
- [x] [Review][Defer] Badge double-count landmine: `countActiveFilters` includes `draft.wilayas` AND the badge adds `wilayaCount` — safe today (`draft.wilayas` is always `[]`), but the instant 3.6 feeds `applied.wilayas` into the draft re-sync the badge becomes 2×N. 3.6 must normalize (wilayaCount is the single wilaya source while the field is wired). [FilterSidebar.tsx, search-service.ts] — deferred, documented in deferred-work.md.
- [x] [Review][Defer] Pre-existing AD-8 risk in the 3.3 key `search.filters.badge` ({count} formats Arabic-Indic in AR via next-intl — same class as the fixed `wilaya_more`); the 3.4 fix covers only the new key. Numbering-system pass needed pre-launch. [messages/ar.json] — deferred, documented in deferred-work.md.

Dismissed as by-design/noise: diacritic-insensitive combobox matching (explicitly out of scope — story notes "plain matching per AC"), Clear-All keeps wilayas active (documented D6 decision + badge test), code-prefix semantics (`'3'` matches codes 30–39 — WilayaTable precedent), registry-file physical classes (`pr-8 pl-1.5 right-2 -ml-1`, side slide-variants — documented registry debt), industries "exactly 35" literal vs by-equality lockstep (amendment 7 documented), Esc-on-closed clearing the query (intentional UX — transient filter text), `+N more` span count (String() fix), dual-instance DOM presence (hidden aside excluded from the a11y tree), rapid-ArrowDown keydown collapse (jsdom focus-manager timing; single-step keyboard paths tested), Enter-toggle-off not asserted in the suite (probed during dev; click-toggle-off + Enter-select + Space-on-focused-option cover the item-press toggle path — amendment 3 wording corrected below).
...
- **Review-stage source verification (Base UI 1.6.0, node_modules)**: (a) native Backspace chip removal exists (`ComboboxInput.js:357-360`, `removalIndex = renderedChipsCount - 1`) — my D3 assumption was wrong, and the double-fire + wrong-chip effects are patched via `preventBaseUIHandler()` + `preventDefault()` (verified: 4-chip test passes with exactly one call); (b) Esc-with-closed-popup wipes the selection (`:344-349`) — intercepted; (c) the internal keydown handler checks `event.which === 229` for IME only AFTER its Backspace branch, but real browsers protect it via the non-empty composing input — my handler's composition guard suppresses it synthetically (suppressBaseUI without preventDefault).
...
- Review-stage: `vi.mocked(useLocale)` requires a vi.fn mock — `mocks.ts` factory changed `useLocale` to `vi.fn(() => 'en')` (harmless default preserved; enables the AR-locale render test).
...
- i18n: +4 keys (`wilaya_label`, `wilaya_remove`, `wilaya_clear`, `wilaya_more`) − 1 (`wilaya_soon`) ×3 locales → 397 keys/locale, parity green. Reused `search.filters.wilaya` (group label), `search.filters.wilaya_placeholder` (input placeholder), `trust.wilayas.no_results` (empty state — the key lives under the trust namespace, verified via the i18n-shape test).
...
- Dev-stage amendments vs story spec: (1) popup opens on the press sequence in jsdom (helper), (2) `autoHighlight` removed (no-op with `filter={null}`), (3) keyboard toggle coverage: Base UI's `'item-press'` toggles on click, Enter (highlighted option) and Space (focused option) — the suite asserts click-toggle-off, Enter-select and Space-on-focused-option; Enter-toggle-off determinism in jsdom is limited by the focus-manager's rAF-timed highlight (rapid keydowns collapse), verified during dev probing and via the click-toggle-off test on the same code path; (4) chip remove buttons are custom native buttons inside stock `ComboboxChip` (`showRemove={false}`) for 44px touch targets; (5) chips container uses the raw Base UI `Chips` primitive for the anchor ref (registry wrapper can't forward refs on React 18); (6) integration tests Esc-close the popup before touching the sidebar (Base UI inerts the background while open); (7) parity tests assert count/ids/names lockstep via equality with the backend mirrors (not a frozen 35) so ops additions stay green; (8) the usage-site `max-h-70` REPLACES the registry's `max-h-[min(calc(--spacing(72)---spacing(9)),...)]` via twMerge — effective 280px per DESIGN.md (the override is required, not cosmetic).
...
- `frontend/src/test/mocks.ts` — UPDATE (elementFromPoint narrowed; `useLocale` now a vi.fn for per-test overrides)
- `frontend/src/app/globals.css` — UPDATE (+`@utility no-scrollbar` — registry ComboboxList dependency)
...
- `frontend/src/__tests__/i18n-shape.test.ts` — NEW (4 tests — every rendered key resolves ×3 locales + interpolation params)
...
- 2026-08-05: Code review (3 parallel layers: Blind Hunter → Edge Case Hunter → Acceptance Auditor — 33 raw findings → 13 patches + 5 deferred + 11 dismissed). Source-verified Base UI facts changed the D3 record: native Backspace chip removal (double-fire + wrong-chip beyond CHIP_LIMIT), Esc-with-closed-popup selection wipe, IME composition hazard. Patches: native-handler suppression (Backspace/Delete + closed-Esc + composition), `trust.wilayas.no_results` key-path fix (mock-masked runtime MISSING), popup aria-label, `data-highlighted:bg-muted`, Western-digit `wilaya_more` count, clear-button `size-11 md:size-4` + focus-visible rings, normalized/deduped `selected` (unknown + duplicate codes), leading-zero code matching, parity hardening (bidirectional + duplicate + all-3-names + trailing-content), `no-scrollbar` utility. Defers (deferred-work.md): dual-instance query state, coordinate-ignoring elementFromPoint, popup-outlives-anchor on viewport change, badge 2×N for 3.6, pre-existing `badge` key AD-8 risk. Post-review gates: 284 frontend tests green (270 + 14), lint 0 / typecheck 0 / check:i18n ✓; backend 354 / ruff 0 / mypy strict 0. Status → done; sprint 3-4 → done (epic-3 stays in-progress).
