...
- RC-2 (interactive review, user decision 2026-08-09): routing reorganized into the `apps/billing/urls/` package — `urls/billing.py` (API) + `urls/webhooks.py` (webhook) replace the old `urls.py`/`webhooks_urls.py` pair; `config/urls.py` includes updated to the tuple+namespace form on BOTH mounts (the stop-3 Low namespace-consistency candidate RESOLVED by this). Post-RC-1 the webhook URL module had lost its technical reason for separation (webhooks.py is no longer celery-imported), so the split is now purely semantic (public Chargily surface vs authenticated API) and the naming debt (`webhooks_urls.py` — the only non-`urls.py` route module in the repo) is gone. Verified: only `config/urls.py` referenced the old modules (no reverse()/test imports); gates 776 pytest / ruff 0 / mypy strict 0 (33 files) / check clean / makemigrations clean; `reverse('billing:create-checkout')` + `reverse('billing_webhooks:chargily')` both resolve.
...
- `backend/apps/billing/urls/billing.py` (NEW — Task 2/RC-2: billing API routes, app_name='billing'; 5.6 adds status polling here)
- `backend/apps/billing/urls/webhooks.py` (NEW — Task 3/RC-2: webhook route, app_name='billing_webhooks')
- `backend/apps/billing/urls/__init__.py` (NEW — RC-2: routing package docstring)
...
- 2026-08-09: RC-2 applied (interactive code review, user decision — routing reorganized into the `apps/billing/urls/` package: `urls/billing.py` + `urls/webhooks.py` replace `urls.py`/`webhooks_urls.py`; both `config/urls.py` includes now use the tuple+namespace form — the stop-3 Low namespace candidate resolved). Gates: 776 pytest / ruff 0 / mypy strict 0 (33 files) / check clean / makemigrations clean; reverse() smoke verified. Committed as its own review-patch commit.
...
- [x] [Review][Patch] RC-2 — reorganize routing into the `apps/billing/urls/` package (interactive review, user decision 2026-08-09): `urls/billing.py` (authenticated API — 5.6's status polling lands here) + `urls/webhooks.py` (public Chargily surface) replace the `urls.py`/`webhooks_urls.py` pair; both `config/urls.py` mounts now use the tuple+namespace include form — the stop-3 Low namespace-consistency candidate is RESOLVED by this. Post-RC-1 the webhook module had lost its technical separation reason (webhooks.py is no longer celery-imported), so the split is semantic and the `webhooks_urls.py` naming debt is gone. Verified: only config/urls.py referenced the old modules; gates 776 pytest / ruff 0 / mypy strict 0 (33 files) / check clean / makemigrations clean; reverse() smoke test both names. [backend/apps/billing/urls/, backend/config/urls.py]
