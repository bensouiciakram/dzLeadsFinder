...
- [x] **Task 1: Backend — `industry` sort whitelist extension** (AC: each column header sortable — the 3.2 companies whitelist lacks `industry`; D2)
  - [x] 1.1 RED: `backend/apps/search/tests/test_company_search.py` — UPDATE — sort `industry:asc`/`industry:desc` orders by industry name (English name — stable, not locale-keyed; nulls LAST in both directions via the universal `nulls_last=True` contract); `industry` accepted by `parse_sort` for companies; REJECTED for people (endpoint-scoped whitelist precedent).
  - [x] 1.2 GREEN: `backend/apps/search/filters.py` + `views.py` — UPDATE — add `industry` to the companies sort whitelist + ORM mapping (`F('industry__name_en')` with `nulls_last=True`, same `_order_by` pattern as the existing keys). Do NOT touch the people whitelist.
  - [x] 1.3 Run backend gates (pytest/ruff/mypy) — green (352 + new).

- [x] **Task 2: AD-20 adoption — QueryClientProvider + client config** (authorized; AD-20; gate PASSED in 3.4)
  - [x] 2.1 RED: `frontend/src/__tests__/providers-query.test.tsx` — NEW — render `<Providers>` with a probe child calling `useQueryClient()` (non-null) + a `useQuery` resolving via the existing mocked `@/lib/api/auth-service` (or a direct fetch-free queryFn); assert the client exists and the query resolves. Mirrors the tanstack-query-gate smoke but through the REAL Providers tree.
  - [x] 2.2 GREEN: `frontend/src/components/providers/Providers.tsx` — UPDATE — mount `QueryClientProvider` with a MODULE-SCOPED `QueryClient` (stable across renders; `defaultOptions: { queries: { retry: false, refetchOnWindowFocus: false } }`) as the OUTERMOST provider. `retry: false` is a QUOTA contract: a transient error must never auto-retry a search (each retried success increments `daily_usage` again — Q8 double-burn); the 3.3 review's "stale-results-hidden-on-error" + one-query-per-Apply semantics stay.
  - [x] 2.3 Keep `frontend/src/__tests__/tanstack-query-gate.test.tsx` untouched (smoke guard stays green).

- [x] **Task 3: HTTP timeout/abort + typed result rows** (deferred-work 3.3 "axios timeout/abort" — 3.5-OWNED; AD-19/AD-20)
  - [x] 3.1 RED: `frontend/src/__tests__/search-service.test.ts` — UPDATE — assert the shared axios instance carries a `timeout` (config-level, e.g. 20000); `searchPeople/searchCompanies` accept an optional `signal?: AbortSignal` and forward it in the axios config (`params` + `signal`); `SearchResult<T>` generic + `PeopleResultRow`/`CompanyResultRow` interfaces with EXACTLY the 3.2 row keys (people: `id, name, role, company_name, wilaya_code, wilaya_name, revealed`; companies: `id, name, industry, industry_id, wilaya_code, wilaya_name, size_band, people_count`).
  - [x] 3.2 GREEN: `frontend/src/lib/api/http-client.ts` — UPDATE — add `timeout` to the default axios config (single shared value; all AD-19 services inherit). `frontend/src/lib/api/search-service.ts` — UPDATE — typed row interfaces, `SearchResult<T>` generic, optional `signal` param on both methods passed through to axios.

- [x] **Task 4: i18n keys ×3 locales** (AC literals; AD-8)
  - [x] 4.1 `frontend/messages/en.json` — UPDATE — AMEND `search.results.empty` value → "No leads match your filters. Try broadening your wilaya or industry selection." (AC literal). NEW keys:
  ...
  - [x] 4.2 Mirror ALL changes in `fr.json` + `ar.json` (Arabic: no uppercase transforms; Western Arabic numerals only in interpolations — AD-8). `npm.cmd run check:i18n` must pass (en = source of truth; identical key counts ×3).

- [x] **Task 5: ResultsTable — TDD** (ACs 1–3, 8; D1, D2, D7, D8)
  - [x] 5.1 RED: `frontend/src/__tests__/results-table.test.tsx` — NEW — pure presentational; props `{ tab, rows, sort, onSortChange }`. Suite (i18n KEYS via the mocked `useTranslations`, never values):
    ...
    - **rows**: 48px (`h-12` class), `border-b` only, hover `hover:bg-muted`; header `text-small`/600/muted-foreground.
...
- [x] **Task 6: ResultsTableStackedRow — TDD** (AC 4; D1)
  ...
  - [x] 6.2 GREEN: `frontend/src/components/search/ResultsTableStackedRow.tsx` — NEW — cards container `md:hidden` (table wrapper is `hidden md:block` — the responsive split lives at the SearchPage integration, not inside either component); per-record card as specced; tokens only; logical CSS only.

- [x] **Task 7: SearchPage → useQuery + results area integration — TDD** (ACs 5–7; D3, D4, D5, D6; AD-20 consumption; deferred-work (b) live-region move)
  - [x] 7.1 RED: `frontend/src/__tests__/search-page.test.tsx` — UPDATE — add a local `renderPage` helper that wraps `<SearchPage tab=... />` in `QueryClientProvider` with a FRESH QueryClient per test (needed once SearchPage consumes useQuery). Keep all 16 existing assertions semantically intact — they become the regression net for the AD-20 refactor:
...
- [x] **Task 8: ActiveFilterChips + applied contract + badge normalization + dual-instance combobox query — TDD** (deferred-work 3.3 "applied re-sync" + 3.4 badge landmine + 3.4 dual-instance; scope resolution: chips ARE 3.5's — the 3.3 defer names "3.5 (chips)")
  - [x] 8.1 RED: `frontend/src/__tests__/active-filter-chips.test.tsx` — NEW — props `{ filters: StagedFilters, onPatch: (patch: Partial<StagedFilters>) => void }`. Suite:
...
- [x] [Review][Patch] Wilaya chip removal was silently reverted on Apply: chips staged into `draft.wilayas` but `runSearch` merged `{ ...filters, wilayas }` from SearchPage state — the combobox + badge kept the removed code and the payload re-included it. [SearchPage.tsx, ActiveFilterChips.tsx] — FIXED: chips now emit REMOVE EVENTS (`onRemove(facet, value)`); SearchPage syncs the wilayas state on wilaya removals; FilterSidebar applies remove semantics to the draft (`removeFacetValue` pure helper); round-trip test (payload + combobox + badge all drop the removed wilaya).
- [x] [Review][Patch] Sequential chip removals of the same facet clobbered each other (each patch was computed from the stale `applied` snapshot with replace semantics — removing A then B re-added A). [ActiveFilterChips.tsx, FilterSidebar.tsx] — FIXED by the remove-event contract: each event mutates the draft by exactly one value; cumulative-removal test.
- [x] [Review][Patch] A chip removal could silently wipe concurrent sidebar edits to the same facet (replace semantics overwrote staged additions). [FilterSidebar.tsx] — FIXED by remove semantics: sidebar-staged values survive; test.
- [x] [Review][Patch] Superseded in-flight searches were not cancelled (sort/page/Apply during a pending query left the old request running to completion). [SearchPage.tsx] — FIXED: `queryClient.cancelQueries({ queryKey: ['search', tab] })` in `beginSearch` (axios abort via the query signal). NOTE: the server-side count may still land (Q8 — the count is the server's business; the client no longer waits on or consumes the superseded response).
- [x] [Review][Patch] Cycling a column to "none" announced "Sorted by {column}, default order" while the server actually applies `name:asc`. [SearchPage.tsx] — FIXED: the default-sort announcement always names the name column (the real default); the header chevron/aria-sort honestly show no user-affirmed sort.
- [x] [Review][Patch] Truncated sets (>1,000) rendered a last page the backend rejects (`(page-1)*100 >= 1000` → 400 `page_out_of_range` — page 11 of an 1,050-result set always errored). [SearchPage.tsx] — FIXED: `totalPages` capped at `MAX_NAVIGABLE_PAGES = 10`, consistent with the truncation notice ("only the first 1,000 results"); test.
- [x] [Review][Patch] `search.filters.badge` sr-only announcement interpolated a raw count → Arabic-Indic digits in AR (pre-existing deferred AD-8 item, 3.4 defer — closed here). [FilterSidebar.tsx] — FIXED: `String(badgeCount)`.
- [x] [Review][Patch] `sort` state survived Clear All — the next search silently inherited the old sort + chevron. [SearchPage.tsx] — FIXED: `setSort(null)` in `handleClearAll`; test.
- [x] [Review][Patch] Skeleton rows were announced as blank table structure (no aria-hidden). [ResultsTable.tsx] — FIXED: `aria-hidden` on skeleton rows + NEW mobile skeleton cards (`skeleton-card`, `md:hidden`, aria-hidden) so the stacked layout also shows a loading skeleton; tests.
- [x] [Review][Patch] A stale sort announcement could resurface after a failed retry. [SearchPage.tsx] — FIXED: announcement cleared on query error.
- [x] [Review][Patch] The mobile drawer had no Clear All affordance (mobile users could only clear per-group). [FilterSidebar.tsx] — FIXED: Clear All button in the DrawerHeader, forwards via `onClearAllRequest`; test.
- [x] [Review][Patch] Empty-string `company_name` rendered a broken link with an empty label. [views.py, ResultsTable.tsx, ResultsTableStackedRow.tsx] — FIXED: backend `(company.name or None)` + frontend falsy guard; backend test.
- [x] [Review][Patch] Mixed Latin/Arabic strings (e.g. "SARL برج بوعريريج") were wrapped wholesale in `dir="rtl"`, scrambling the Latin prefix (bidi). [ResultsTable.tsx, ResultsTableStackedRow.tsx, ActiveFilterChips.tsx] — FIXED: `isArabic` now requires NO Latin letters (pure-Arabic fragments only get the wrapper); tests.
- [x] [Review][Patch] `RevealSlot` was `aria-disabled` yet focusable and silently activatable. [ResultsTable.tsx, ResultsTableStackedRow.tsx] — FIXED: native `disabled` + `disabled:opacity-50`.
- [x] [Review][Patch] Unknown `size_band` values (free-text field) rendered the raw i18n key (a real next-intl MISSING). [ResultsTable.tsx, ActiveFilterChips.tsx, ResultsTableStackedRow.tsx] — FIXED: `bandLabelKey` returns null for unknown bands; cells/chips render the raw value; tests.
- [x] [Review][Patch] No sort tie-breaker → unstable pagination on tied sort values (a row could appear on two pages). [views.py] — FIXED: universal secondary `'id'` sort key in `_order_by` (all sort fields); determinism test.
- [x] [Review][Patch] Last table row lost its bottom border — the registry `TableBody` `[&_tr:last-child]:border-0` (specificity 0,2,1) beats `.border-b`; the story's completion record CLAIMED a fix that was not in the code. [ResultsTable.tsx] — FIXED: `TableBody className="[&_tr:last-child]:border-b"` (AC literal: every row 1px bottom border); completion record corrected.
- [x] [Review][Patch] People stacked card dropped the company line entirely when `company_name` was null (the table renders an em-dash — inconsistent reflow). [ResultsTableStackedRow.tsx] — FIXED: the company line always renders (em-dash when null); test.
- [x] [Review][Patch] Companies stacked card omitted the People-count field ("same data as the table" AC). [ResultsTableStackedRow.tsx] — FIXED: labeled count line with tabular-nums; test.

Dismissed as by-design/noise: industry sort fixed to English names (deterministic cross-locale ordering — documented D2; locale-keyed collation would be order-dependent), unknown/obsolete codes in applied filters have no chip (data is parity-locked — chips derive from the same files as the backend), typed-but-unselected combobox text persists after Apply (transient filter text — 3.4 dismissal precedent), `submitNonce` fresh-query semantics (documented amendment 1 — one Apply = one counted search per FR-7; re-apply of identical filters re-queries rather than cache-hitting, preserving the stale-results-hidden-on-error contract), one-query-per-sort/page quota consumption (documented D2 — every sort transition counts).
...
- **twMerge 3.6 eats the token font-size classes**: `text-small`/`text-caption`/`text-data`/`text-title`/`text-body`/`text-headline`/`text-display` are NOT in tailwind-merge's font-size group (it only knows the default scale), so they fall into the text-COLOR group and get dropped whenever a `text-foreground`/`text-muted-foreground` class is present (the registry `th` ships `text-foreground`). Root fix in `src/lib/utils.ts`: `extendTailwindMerge({ extend: { classGroups: { 'font-size': [<the 7 token classes>] } } })` — verified with direct node probes; benefits every future cn() usage. Workaround considered and rejected: `text-(--font-size-small)` (empirically also collides in twMerge 3.6) and hardcoded `text-[14px]` (AD-2 violation).
- **Disabled TanStack queries are `status: 'pending'`**: with `enabled: submitted !== null`, the pre-submit query reports `isPending: true` — a naive `phase = isPending ? 'loading' : 'idle'` made the Apply button show the loading label and break every suite. Phase now requires `submitted !== null && query.isPending`.
- **The `submitNonce` in the query key**: re-applying IDENTICAL filters with plain `['search', tab, filtersJson, page, sort]` hits the cache (no network, no error) — which would silently break the 3.3 "stale-results-hidden-on-error" and re-apply contracts. Every user action (Apply/sort/page) bumps a nonce in the key → always a fresh query (one Apply = one counted search, FR-7); a double-click Apply still dedupes to ONE call because React batches both `setSubmitNonce(fn)` updaters (single final key).
- **People rows needed `company_id`**: the AC-literal company-name link in the People table requires the company id — the 3.2 row shape lacked it. Added `company_id` (str | null) to `_people_row` + the row-shape test + `PeopleResultRow` (additive; no consumer breakage).
- **Drawer popups portal to `document.body`**: the dual-instance combobox test must query `document.querySelectorAll('input[role="combobox"]')` (container-scoped queries miss the portaled drawer copy).
- **`getByText` with split text**: the wilaya cell renders `31 — Oran` across two inner spans — exact-string `getByText` cannot match it (getNodeText only sees direct text nodes); tests use `getAllByText((_, el) => el?.textContent === '31 — Oran')` / cell-scoped `querySelectorAll` instead.
- **Multiple null cells per row**: null role + null company in the same row produce 2 em-dashes — assertions use `getAllByText('—')`, not `getByText`.
- **Registry `text-left` on th**: overridden to `text-start` via cn (twMerge drops the physical class) — verified by the RTL no-physical-classes test over the RENDERED class list.
- Pre-existing noise (untouched): vitest CJS banner, jwt InsecureKeyLengthWarning in backend tests.

### Completion Notes List

- `ResultsTable.tsx` (NEW): stock base-nova `table` + `skeleton` (registry, zero new deps); per-tab column sets (people name/role/company/wilaya/reveal — companies name/industry/wilaya/size/people-count); sortable header buttons with `sortCycle` (none→asc→desc→none), `aria-sort` on the active th only, chevrons `ChevronsUpDown/ChevronUp/ChevronDown` (data-state + muted for none); pure helpers `sortCycle`, `bandLabelKey`, `columnLabelKey`, `isArabic`; 48px rows `h-12` + `border-b` + `hover:bg-muted` (twMerge override of the registry `hover:bg-muted/50`); `MaybeArabic` per-fragment `lang="ar" dir="rtl"`; company names = real next/link `/companies/:id` (rows NOT clickable); nulls → muted em-dash; wilaya cell `31 — Oran` with tabular-nums code; `tabular-nums` on people_count; reveal column = inert `aria-disabled` slot for Epic 4.2; `skeleton` prop renders the real header + 5 Skeleton rows (zero layout shift); table wrapper `hidden md:block`; logical CSS only.
- `ResultsTableStackedRow.tsx` (NEW): per-record cards `rounded-lg border border-border bg-card p-gutter`, `md:hidden`; lead name `text-title`; meta `text-small text-muted-foreground` (people: role + company link; companies: industry + size band key); wilaya line; full-width reveal slot `w-full min-h-11`; same order/data as the table.
- `ActiveFilterChips.tsx` (NEW): chips per active value in spec order (industries→wilayas→seniorities→sizes→keyword→unknown-size); `rounded-full bg-muted`; wilaya chips `31 — Oran` with tabular-nums code span; remove buttons real `<button>` `size-11 md:size-4`, aria-label `search.results.chip_remove` + name; `onPatch` = replace-semantics partial patch; hidden when empty.
- `SearchPage.tsx` (REWRITE): AD-20 `useQuery` consumer (queryKey `['search', tab, filtersJson, page, sort, submitNonce]`, queryFn via searchService with `signal`, `enabled`); phases from query states (pending→loading, 429→rate_limited with detail, other errors→error with Retry=refetch); `applied` set on success (stable identity — the submitted object); sort state → `sort=field:asc|desc` (default `name:asc`); pagination `total > 100` with Previous/Next (`rtl:rotate-180` chevrons), `aria-current="page"` indicator, page reset on filter/sort change; live region moved to the count/status line (`data-testid="results-status"` aria-live=polite) with sr-only `role="status"` sort/page announcements; skeletons via the table's skeleton prop + `aria-busy`; empty broaden + `search.results.clear_all` → page-level reset (draft + wilayas, no query); sidebar Clear All wired via `onClearAllRequest`; shared `wilayaQuery` state; chips above the table from `applied`; `results-slot` contract preserved; inFlightRef removed.
- `FilterSidebar.tsx` (UPDATE): optional `stagedPatch`/`clearNonce`/`onClearAllRequest` props; badge normalization `countActiveFilters({ ...draft, wilayas: [] }) + wilayaCount` (wilayaCount = single source — the 3.4 landmine closed); applied resync with dirty-guard (`dirtyRef` set by user edits; resync only when untouched — staged-in-flight edits never clobbered); Clear All resets draft + fires `onClearAllRequest`.
- `WilayaCombobox.tsx` (UPDATE): optional controlled `inputValue`/`onInputValueChange` (query state liftable; backward compatible).
- `Providers.tsx` (UPDATE): module-scoped `QueryClient` (`retry: false`, `refetchOnWindowFocus: false` — the retry:false is a QUOTA contract) mounted as the outermost provider.
- `http-client.ts` (UPDATE): shared axios `timeout: 20000` (all AD-19 services inherit — the 3.3 deferred hung-request guard).
- `search-service.ts` (UPDATE): `PeopleResultRow` (+`company_id`) / `CompanyResultRow` types, `SearchResult<T>` generic, optional `signal` on both search methods.
- Backend (UPDATE): companies sort whitelist + `F('industry__name_en')` nulls-last mapping (AC all-columns-sortable); `_people_row` + `company_id`; row-shape + sort tests (+3).
- i18n: +9 keys (columns.role/company/people_count, sort_asc/desc/default, previous, clear_all, chip_remove) ×3, amended `search.results.empty` ×3 → 406 keys/locale, parity green.
- `lib/utils.ts` (UPDATE): tailwind-merge extension registering the 7 token font-size classes (see Debug Log).
- Tests: 83 new frontend tests (results-table 28, stacked-row 12, chips 10, providers-query 1, search-page +9, filter-sidebar +5, search-service +4, i18n shape additions auto) → 353 total frontend (270 → 353); backend 357 (354 + 3 sort/row tests). Frontend gates: lint 0 / typecheck 0 / check:i18n 406×3 ✓. Backend: ruff 0 / mypy strict 0 / pytest 357 ✓.
- Dev-stage amendments vs story spec: (1) `submitNonce` in the query key (fresh-query semantics; see Debug Log); (2) people rows carry `company_id` (AC link support); (3) tailwind-merge extension in utils.ts (token font-size classes); (4) `text-small` lives on an inner label span rather than the th element (twMerge conflict workaround — the AC visual is identical, tests assert the label span); (5) `TableBody` gets `[&_tr:last-child]:border-b` override so EVERY row keeps its border (AC literal; registry drops the last row's border); (6) chips fixture industries ids corrected to real data (id 4 = Advertising); (7) dual-instance test opens the drawer and queries document.body (portal); (8) the announcement span only renders in the idle phase (sort/page announcements appear with the results, not during the skeleton phase — assertion uses findByText).

### File List

- `_bmad-output/implementation-artifacts/3-5-results-table-stacked-row.md` — UPDATE (this story; status → review)
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — UPDATE (3-5 → in-progress → review)
- `_bmad-output/implementation-artifacts/deferred-work.md` — UPDATE (3.5-resolved items closed; /companies/:id defer recorded)
- `frontend/src/components/search/ResultsTable.tsx` — NEW (sortable table + pure helpers + skeleton mode)
...
- `frontend/src/lib/api/search-service.ts` — UPDATE (typed rows incl. company_id, SearchResult<T>, signal)
- `frontend/src/lib/utils.ts` — UPDATE (tailwind-merge font-size extension)
- `frontend/messages/{en,fr,ar}.json` — UPDATE (+9 keys; amended empty value)
...
- `backend/apps/search/filters.py` + `views.py` — UPDATE (companies industry sort; people company_id)
- `backend/apps/search/tests/{test_company_search,test_people_search,test_filters}.py` — UPDATE (industry sort + company_id tests)
...
- 2026-08-05: Implemented (TDD): RED suites (results-table 28, stacked-row 12, chips 10, providers-query 1, search-service +4) → backend industry sort + company_id rows → AD-20 adoption (Providers QueryClientProvider retry:false; SearchPage useQuery consumer with submitNonce fresh-query semantics) → ResultsTable/StackedRow/Chips components + results area (live region move, skeletons aria-busy, pagination, empty broaden + page-level clear-all, sort announcements) → FilterSidebar contract (stagedPatch/clearNonce/onClearAllRequest + badge normalization + dirty-guard resync) + shared combobox query → i18n +9 keys ×3 + empty amend → GREEN 353 frontend tests (270 + 83), lint 0 / typecheck 0 / check:i18n 406×3 ✓; backend 357 pytest (354 + 3) / ruff 0 / mypy strict 0. Dev-stage amendments recorded (submitNonce fresh-query semantics, company_id in people rows, tailwind-merge font-size extension in utils.ts, label-span text-small, last-row border-b override, chips fixture correction, portal-scoped dual-instance query, announcement in idle phase). Status → review; sprint 3-5 → review; deferred-work 3.5-OWNED items (timeout/abort, live-region move, AD-20 adoption, applied re-sync + chips, badge landmine, dual-instance query) resolved.
- 2026-08-05: Code review (3 parallel layers: Blind Hunter → Edge Case Hunter → Acceptance Auditor — 24 raw findings → 19 patches + 5 dismissed). Patches: chip contract reworked from replace-patches to REMOVE EVENTS (fixes wilaya-chip revert on Apply, sequential-removal clobber, sidebar-edit wipe), `cancelQueries` on submit (superseded in-flight searches aborted), default-sort announcement correction (names the real name:asc default), pagination capped at the 1,000-row navigable limit (truncated last-page 400 fixed), badge sr-only `String()` (closed the pre-existing AD-8 defer), sort reset on Clear All, skeleton aria-hidden + mobile skeleton cards, announcement cleared on error, drawer Clear All button, empty-string company_name guard (backend + frontend), pure-Arabic-only bidi wrapper, native-disabled RevealSlot, unknown size_band raw fallback, universal `'id'` sort tie-breaker, last-row border-b TableBody override (completion record corrected — the claimed fix was absent), stacked-card data parity (company em-dash line + People-count line). Dismissed: English industry sort (deterministic by design), parity-locked invisible codes, transient combobox text, submitNonce fresh-query economics (documented amendment), one-query-per-action quota model (documented D2). Post-review gates: 369 frontend tests green (353 + 16), lint 0 / typecheck 0 / check:i18n ✓; backend 359 pytest (357 + 2) / ruff 0 / mypy strict 0. Status → done; sprint 3-5 → done (epic-3 stays in-progress).
