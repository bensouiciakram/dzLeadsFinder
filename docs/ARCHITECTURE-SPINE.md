...
**Rule:** **TanStack Query v5 is the server-state layer for all data-fetching surfaces starting with Epic 3 (search).** Adoption is deliberately deferred until the first real consumer (story 3-1/3-2 — search backend + endpoints); stories 2.4–2.6 (auth forms, account deletion) ship without it. Queries call the AD-19 service methods via `queryFn` (e.g. `queryFn: () => authService.me()`); the axios interceptor (401 routing, sliding-session refresh) stays in the HTTP layer untouched. Auth/session state remains `SessionProvider`-owned context — it is NOT migrated into the query cache (the Header needs synchronous `useSession()`; login/logout/refresh are imperative flows, not cached data). `QueryClientProvider` mounts in `Providers`; vitest tests wrap components in `QueryClientProvider` with the existing `@/lib/api/auth-service` module mocks. **Adoption prerequisite:** verify TanStack Query v5 works under the current vitest 2.x/Vite-CJS test stack before committing the dependency (same gate that blocked zod v4, AD-18) — if it fails, revisit on a vitest/Vite ESM upgrade. Polling use-cases (billing status-card ≤60s, AD-5/FR-27) use `refetchInterval`; mutation invalidation keeps Credits pill, ledger, and saved searches consistent. **Status:** adopted in Story 3.5 — the search results query is the first real consumer (`hooks/useSearchResults.ts`); the gate test (`tanstack-query-gate.test.tsx`) remains the smoke guard. Convention details for all consumers live in AD-21.

## AD-21: React Query conventions (hooks, key factories, cache tuning)

**Binds:** How every `useQuery`/`useMutation` consumer is structured, keyed, tuned, and documented across Epic 3+.
**Prevents:** Inline React Query calls scattered through components, ad hoc key arrays breaking invalidation/cancellation, per-feature `QueryClient` forks, and divergent loading/error/empty handling between surfaces.

**Rule:**

- **Custom hooks only.** No component calls React Query directly. Each feature owns `frontend/src/hooks/use<Feature>.ts` (queries) / `use<Feature><Verb>.ts` (mutations), returning a typed object (e.g. `{ query, phase, rateLimitMessage, beginSearch }`). Components consume only the hook's return value. The hook owns the `useQuery`/`useMutation` call, `enabled` gating, phase derivation, error mapping (e.g. the 429 message effect), and success side-effects (via an `onSuccess` callback for parent-owned state). Orchestration that combines the hook with other state (announcements, chip resets) lives in the consumer as a thin wrapper over the hook's returned actions.
- **Query key factories.** All keys derive from `frontend/src/lib/queryKeys/<feature>.ts` factories (e.g. `searchKeys.all` / `searchKeys.tab(tab)` / `searchKeys.results(tab, filtersJson, page, sort, nonce)`), typed as `as const` tuples. Invalidation and cancellation always use factory keys — never inline arrays. Prefix-based cancellation: `queryClient.cancelQueries({ queryKey: keys.tab(...) })` (the 3.5 superseded-in-flight-search abort). To extend: add a more specific key to the feature factory; consumers and invalidators pick it up without string drift.
- **Strong typing.** Every `queryFn`/`mutationFn` carries an explicit return type annotation (e.g. `Promise<SearchResult<PeopleResultRow> | SearchResult<CompanyResultRow>>`); no `any` in the data layer; API errors are narrowed with exported type guards (e.g. `isRateLimitError` in `search-service.ts`).
- **enabled gating.** Queries fire only when their inputs are valid and ready: `enabled: submitted !== null`, dependent values present, auth/context available. Disabled queries must not read as "loading" (a disabled query reports `status: 'pending'` — phase derivation must also require the input, e.g. `submitted !== null && query.isPending`).
- **Explicit state handling.** Consumers render all states explicitly — loading (skeletons + `aria-busy`), error (inline retry UI), rate-limited (message + `aria-describedby` on Apply), empty (per-surface empty state), idle-with-data — via a derived `phase`, never `data` truthiness alone.
- **No `placeholderData`.** Deliberately rejected (story 3.5): key changes show skeletons; previous data must never render next to an error state ("stale-results-hidden-on-error", 3.3 review contract). Revisit only as a pagination-only exception with test and story-record amendments.
- **No debouncing for discrete actions.** Search is Apply-not-instant (FR-7 — one Apply = one counted search; sort/page clicks are discrete user actions; the submit nonce dedupes double-triggers). If a live-search surface ever appears, debounce the input→query mapping, never the counted query itself.
- **Cache tuning.** One global `QueryClient` in `Providers.tsx`: `retry: false` (a retried success would double-burn the FR-7 daily quota — Q8 increments after success), `refetchOnWindowFocus: false` (quota), default `staleTime: 0`, default `gcTime` 5 min. Search results stay `staleTime: 0` because a cache hit on re-Apply would silently skip the counted query; the submit nonce already forces a fresh fetch per action. Any per-query override must carry a documented rationale in the hook (e.g. billing polling uses `refetchInterval` ≤60s per AD-5).
- **Mutations.** Same hook-extraction, typing, and error-handling rules. Post-mutation cache updates go through the key factories (`invalidateQueries` / `setQueryData` with factory keys), never ad hoc key arrays. The first real mutation lands with Story 3.6 (saved searches).
- **Centralized error handling.** Session/server errors stay in the HTTP layer (AD-19 interceptor — 401 routing, sliding-session refresh). Per-surface: rate-limit (429) mapping in the feature hook, inline error UI with retry in the consumer. No global error toast — it would fight the inline retry and 429 UX.

**How to add a query or mutation (checklist):** (1) add the key factory entry (or extend the feature factory); (2) create `hooks/use<Feature>.ts` with the `useQuery`/`useMutation`, explicit return types, `enabled` gating, phase, and error mapping; (3) consume only the hook in the component with all four states rendered; (4) set `staleTime`/`gcTime` overrides with a written rationale only if non-default; (5) tests through the component (wrapped in `QueryClientProvider`, fresh client per test) or `renderHook`; (6) run the full gate suites (test / lint / typecheck / check:i18n ×3 locales).
...
| AD-20 | TanStack Query v5 as server-state layer from Epic 3 (queries via AD-19 services; session stays context) | [ADOPTED] |
| AD-21 | React Query conventions: feature hooks + key factories + cache tuning for all AD-20 consumers | [ADOPTED] |
